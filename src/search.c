...
fgrep_status_t search_data(const char *data, size_t len, const char *path,
    ...
    if (opts->count_only || opts->files_with_matches || opts->files_without_match) {
        if (nlen == 1) {
            ...
            for (; pos < end; pos++) if (data[pos] == c) count++;
        } else {
            ...
            while (pos + 32 <= end) {
                ...
                while (combined) {
                    ...
                    size_t p = pos + (size_t)bit;
                    if (memcmp(data + p + 1, needle + 1, nlen - 2) == 0) count++;
                    combined &= combined - 1;
                }
                pos += 32;
            }
            for (; pos + nlen <= len; pos++) {
                if (data[pos] == first && data[pos + nlen - 1] == last &&
                    memcmp(data + pos + 1, needle + 1, nlen - 2) == 0) count++;
            }
        }
    } else {
